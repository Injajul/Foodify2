...
export default function UpdateRestaurant() {
  ...
  useEffect(() => {
    if (singleRestaurant) {
      setFormData({
        ...
        deliveryTime: singleRestaurant.deliveryTime?.toString() || "30",
        deliveryRadius: singleRestaurant.deliveryRadius?.toString() || "",
        openingTime: singleRestaurant.openingTime || "",
        closingTime: singleRestaurant.closingTime || "",
        image: null,
        imagePreview: singleRestaurant.image?.url || null,
      });
    }
  }, [singleRestaurant]);
  ...
  const addCustomCuisine = () => {
   ...
   // Submit: Build FormData → Dispatch thunk → Toast → Reset → Navigate
  const handleSubmit = async (e) => {
    ...
    try {
      // unwrap() allows us to catch success or error like a normal promise
      const result = await dispatch(
        updateRestaurant({ id, formData: fd, token })
      ).unwrap();

      // SUCCESS TOAST
      toast.success(result.message || "Restaurant updated successfully!");

      setFormData(DEFAULT_FORM); // reset form
  ...
  return (
    <div className="min-h-screen overflow-hidden bg-gradient-to-br from-slate-50 via-purple-50 to-pink-50">
      {/* Sticky Header */}
        ...
        <div className="max-w-4xl mx-auto px-6 py-5 flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">
              Fine-tune your business details
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Main Form */}
      <div className="w-full lg:max-w-4xl mx-auto lg:py-10 pb-24 lg:pb-0">
        <motion.div
